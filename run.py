...
def train_baseline():
    ...
    # HYPERPARAM
    dataset_name = 'processedFCU_subsample_ws_6'
    experiment_name = 'subset_1'
    BATCH_SIZE = 35600
...
def train_pyramid():
    ...
    # HYPERPARAM
    dataset_name = 'processedPyramid_subsample_ws_6'
    experiment_name = 'subset_1_gru'
    ...
    path_fig       = os.path.join('figs', dataset_name)
    path_model     = os.path.join('trained_models', dataset_name)
    path_laplacian = os.path.join('data', 'laplacian_subsampled.pt')
    ...
    dataset = DatasetPyramid(path_processed, n_slopeunit=N_SLOPEUNIT, years=range(102, 105), normalize=True)
    train, valid, test = random_split(dataset, [0.7, 0.15, 0.15])
