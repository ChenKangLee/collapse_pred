    ...
    def load(self, path, interval=(94,107), neg_filter_rate=0):
        ...
        for year in range(interval[0], interval[1]):
            ...
            with open(path_collapse, 'rb') as file:
                ...
                # categorize by the supplied bins
                categorized = np.digitize(collapse_arr, self.label_bins, right=True).reshape(-1)

                collapse.append(collapse_arr)
                label.append(np.eye(len(self.label_bins) + 1)[categorized]) # convert to one-hot
        ...
        self.label = torch.from_numpy(np.concatenate(label, axis=0)).float()
